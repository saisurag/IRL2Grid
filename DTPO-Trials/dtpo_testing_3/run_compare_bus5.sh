...
run_one () {
  ...
  local before="/tmp/before_cmp_$tag.txt"
  ...
  PYTHONUNBUFFERED=1 timeout "$tlimit" $PY main.py --alg DTPO --env-id bus5 --action-type topology --difficulty 0 \
      ...
      --warmstart-oracle "$BUS5_ORACLE" --warmstart-oracle-type ppo \
      $extra \
      > "$folder/train.log" 2>&1
  echo "=== train exit=$? for $tag $(date +%H:%M:%S) ==="

  ls checkpoint/*.tar 2>/dev/null | sort > "/tmp/after_cmp_$tag.txt"
  local newlist=$(comm -13 "$before" "/tmp/after_cmp_$tag.txt")
  local NEW=""
  ...
  bash dtpo_testing/finalize_run.sh "$folder" "$before" "bus5 seed$seed ($tag)" \
      2>&1 | grep -vE "FutureWarning|warnings.warn" || echo "!! finalize failed for $tag"

  local ck=$(ls "$folder"/DTPO_*.tar 2>/dev/null | head -1)
  if [ -n "$ck" ]; then
    PYTHONUNBUFFERED=1 $PY fidelity_eval.py --tree-ckpt "$ck" --oracle-ckpt "checkpoint/$BUS5_ORACLE.tar" \
        --episodes 10 --seed 0 --out "$folder/fidelity.json" 2>&1 | grep -E "\[fidelity\]" || echo "  (fidelity skipped)"
  fi
  echo "=== DONE $tag $(date +%H:%M:%S) ==="
}

echo "########## bus5 A/B compare $(date +%H:%M:%S) ##########"

echo "########## A + B in PARALLEL $(date +%H:%M:%S) ##########"
run_one "$OUT/bus5_baseline_seed0" 0 150 100000 64 86400 \
    --dtpo-ppo-clip 0.2 --dtpo-policy-updates 10 --dtpo-anneal-lr True \
    > "$OUT/bus5_baseline_seed0.runlog" 2>&1 &
PIDA=$!
run_one "$OUT/bus5_dagger_seed0" 0 150 100000 64 43200 \
    --warmstart-dagger-rounds 5 \
    > "$OUT/bus5_dagger_seed0.runlog" 2>&1 &
PIDB=$!
echo "A pid=$PIDA  B pid=$PIDB  (tail bus5_*_seed0.runlog for per-run progress)"
wait $PIDA $PIDB

echo
echo "===== bus5 A/B COMPARE COMPLETE $(date +%H:%M:%S) ====="
echo "--- baseline reference: bus5_hybrid_seed0 = 37.04% honest, on_tree surv 0.059 fid 0.0 ---"
for f in "$OUT"/bus5_baseline_seed0 "$OUT"/bus5_dagger_seed0; do
  printf "%-28s " "$(basename "$f")"
  grep -E "HONEST BEST" "$f/selection.txt" 2>/dev/null | sed 's/.*HONEST BEST = //' || echo "(no result)"
done
