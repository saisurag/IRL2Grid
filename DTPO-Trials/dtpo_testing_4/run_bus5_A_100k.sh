...
run_one () {
  ...
  bash dtpo_testing/finalize_run.sh "$folder" "$before" "bus5 seed$seed ($tag, batch$batch, runA-optimizer)" \
...
echo "########## dtpo_testing_4 bus5 run-A batch100k, seeds 1+2 in PARALLEL $(date +%H:%M:%S) ##########"
run_one "$OUT/bus5_A100k_seed1" 1 150 100000 64 172800 \
    --dtpo-ppo-clip 0.2 --dtpo-policy-updates 10 --dtpo-anneal-lr True \
    > "$OUT/bus5_A100k_seed1.runlog" 2>&1 &
P1=$!
run_one "$OUT/bus5_A100k_seed2" 2 150 100000 64 172800 \
    --dtpo-ppo-clip 0.2 --dtpo-policy-updates 10 --dtpo-anneal-lr True \
    > "$OUT/bus5_A100k_seed2.runlog" 2>&1 &
P2=$!
echo "seed1 pid=$P1  seed2 pid=$P2"
wait $P1 $P2

echo
echo "===== dtpo_testing_4 bus5 run-A batch100k COMPLETE $(date +%H:%M:%S) ====="
echo "--- ref: bus5 run-A seed0 @ batch20000 = 100% honest ---"
for f in "$OUT"/bus5_A100k_seed1 "$OUT"/bus5_A100k_seed2; do
