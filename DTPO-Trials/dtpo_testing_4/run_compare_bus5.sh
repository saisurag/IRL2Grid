...
run_one () {
  ...
  bash dtpo_testing/finalize_run.sh "$folder" "$before" "bus5 seed$seed ($tag, batch$batch)" \
...
echo "########## dtpo_testing_4 bus5 A/B (batch 20000), A + B in PARALLEL $(date +%H:%M:%S) ##########"

run_one "$OUT/bus5_baseline_seed0" 0 150 20000 64 43200 \
    --dtpo-ppo-clip 0.2 --dtpo-policy-updates 10 --dtpo-anneal-lr True \
    > "$OUT/bus5_baseline_seed0.runlog" 2>&1 &
PIDA=$!
run_one "$OUT/bus5_dagger_seed0" 0 150 20000 64 21600 \
    --warmstart-dagger-rounds 5 \
    > "$OUT/bus5_dagger_seed0.runlog" 2>&1 &
PIDB=$!
echo "A pid=$PIDA  B pid=$PIDB  (tail dtpo_testing_4/bus5_*_seed0.runlog for per-run progress)"
wait $PIDA $PIDB

echo
echo "===== dtpo_testing_4 bus5 A/B COMPARE COMPLETE $(date +%H:%M:%S) ====="
echo "--- baseline ref: dtpo_testing_3/bus5_hybrid_seed0 = 37.04% honest, on_tree surv 0.059 fid 0.0 ---"
