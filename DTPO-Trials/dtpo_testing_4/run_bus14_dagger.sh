...
run_one () {
  ...
  bash dtpo_testing/finalize_run.sh "$folder" "$before" "bus14 seed$seed ($tag, batch$batch, dagger)" \
...
echo "########## dtpo_testing_4 bus14 DAgger seed0 (batch 20000, 64 leaves) $(date +%H:%M:%S) ##########"
run_one "$OUT/bus14_dagger_seed0" 0 150 20000 64 43200 \
    --warmstart-dagger-rounds 5

echo
echo "===== dtpo_testing_4 bus14 DAgger COMPLETE $(date +%H:%M:%S) ====="
echo "--- bus14 baseline floor ~13% (dtpo_testing_3) ---"
grep -E "HONEST BEST" "$OUT/bus14_dagger_seed0/selection.txt" 2>/dev/null | sed 's/.*HONEST BEST = /bus14_dagger_seed0  /' || echo "(no result)"
