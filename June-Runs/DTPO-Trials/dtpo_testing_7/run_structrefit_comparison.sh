...
run_k () {
  local k=$1
  local out=$DIR/bus14_structrefit_K${k}_seed100
  local marker=$out/.start; : > "$marker"
  echo "=== START structrefit K=$k $(date +%T) ==="
  ...
  $PY plot_dtpo_curve.py --log "$out/train.log" --out "$out/curve.png" --title "structrefit_K${k}" >> "$out/train.log" 2>&1
  echo "=== DONE K=$k $(date +%T) ==="
}

run_k 1
run_k 10
echo "===== STRUCTREFIT COMPARISON COMPLETE $(date +%T) ====="
