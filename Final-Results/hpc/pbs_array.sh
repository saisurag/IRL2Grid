...
python -u "$ROOT/honest_eval_any.py" --ckpt "$CKPT" --total "$HONEST_TOTAL" \
       --eval-pool held  > honest_eval.log 2>&1
echo "=== honest eval (held-out) exit=$? ==="
grep "^RESULT" honest_eval.log

python -u "$ROOT/honest_eval_any.py" --ckpt "$CKPT" --total "$HONEST_TOTAL" \
       --eval-pool train > honest_eval_indist.log 2>&1
echo "=== honest eval (in-distribution) exit=$? ==="
sed -i 's/^RESULT /RESULT_INDIST /' honest_eval_indist.log
grep "^RESULT_INDIST" honest_eval_indist.log
