...
def main():
    ...
    for name in sorted(os.listdir(a.runs_dir)) if os.path.isdir(a.runs_dir) else []:
        ...
        # run-name convention from gen_experiments.py:
        #   <envtag>_<method>[_L<leaves>]_s<seed>   (envtag may carry -d1)
        ...
        # v2: pbs_array.sh scores the SAME checkpoint twice -- honest_eval.log is
        # the HELD-OUT pool (generalisation, the headline) and honest_eval_indist.log
        # is the training-side pool (in-distribution, the RL2Grid convention).
        # Pull the second one in as indist_* columns so one CSV carries both.
        indist = None
        ilog = os.path.join(d, "honest_eval_indist.log")
        if os.path.isfile(ilog):
            with open(ilog) as f:
                for line in f:
                    if line.startswith("RESULT_INDIST ") or line.startswith("RESULT "):
                        indist = parse_kv(line.replace("RESULT_INDIST ", "RESULT ", 1))
        ...
        rows.append({
            "name": name, "status": "ok",
            "indist_select": (indist or {}).get("select"),
            "indist_report": (indist or {}).get("report"),
            "indist_frac100": (indist or {}).get("frac100"),
    ...
    fields = ["name", "status", "envtag", "method", "leaves_budget", "seed", "alg", "env",
              ...
              "indist_select", "indist_report", "indist_frac100"]
    ...
    # pivot: median [IQR] honest REPORT survival + mean bigjumps per cell
