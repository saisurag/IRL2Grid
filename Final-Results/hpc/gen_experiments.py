#!/usr/bin/env python3
"""
Generate the experiment lists for the RL2Grid interpretable-policy empirical
study (methods x envs x seeds x leaves x difficulty), one file per env so each
PBS array can request env-appropriate resources.

Line format (parsed by pbs_array.sh):
    name|script|honest_eval_total|cli args...
Paths inside args use the __ROOT__ token, substituted with the repo root by the
array job (jobs run inside their own working dir so every run gets a private
checkpoint/ folder -> no filename races between concurrent subjobs).
...
Tiers
  core      6 methods @ 16 leaves               -> headline method comparison
  ablation  gate/replay decomposition + scratch_full
  frontier  {8,32,64} leaves for tree methods
  hparam    replay depth M in {2,8}, gate patience 3
  stress    bus14 difficulty 1, scratch variants only (no d1 oracle exists yet)

Usage:
  python gen_experiments.py                     # bus5 + bus14, seeds 100-104
  python gen_experiments.py --envs bus5 --seeds 100 101 102
  python gen_experiments.py --tiers core ablation
"""
...
EVAL_TOTAL  = {"bus5": 10, "bus14": 40, "bus36-M": 40, "bus118-M": 30}
HONEST_TOTAL = {"bus5": 10, "bus14": 80, "bus36-M": 80, "bus118-M": 60}


HOLDOUT = {"bus5": 2, "bus14": 4, "bus36-M": 4, "bus118-M": 4}
EVAL_SEED = 12345

def v2_flags(env):
    return (f"--chronics-mode multifolder --chronic-holdout {HOLDOUT[env]} "
            f"--eval-seed {EVAL_SEED} --eval-norm-reset per-chronic")
...
WARMSTART = ("--warmstart-oracle {oracle} --warmstart-temperature 0.5 "
             "--warmstart-dagger-rounds 0 --warmstart-seeds 20")
...
def viper_line(env, seed, leaves, diff=0):
    return (v2_flags(env) + " " +
            f"--alg VIPER --env-id {env} --action-type topology --difficulty {diff} "
...
def dagger_line(env, seed, diff=0):
    return (v2_flags(env) + " " +
            f"--alg DAGGER --env-id {env} --action-type topology --difficulty {diff} "
...
def distill_line(env, seed, leaves, diff=0):
    return (v2_flags(env) + " " +
            f"--env-id {env} --difficulty {diff} --oracle {ORACLES[env]} "
        ...
        for s in a.seeds:
            ...
            if "ablation" in a.tiers:
                if has_oracle:
                    ...
                    add(f"{env}_dtpo-gate-K1_L{L}_s{s}", "main.py",
                        dtpo_line(env, s, L, warm=True, gate=True, replay_iters=1, K=1))
