...
class P3O:
    """Implementation of the Penalized Proximal Policy Optimization (P3O) algorithm for training a safe agent in a given environment: https://arxiv.org/pdf/2205.11814.
    """
    ...
    def __init__(self, envs: gym.Env, run_name: str, start_time: float, args: Dict[str, Any], ckpt: CheckpointSaver):
        ...
        try:
            for iteration in range(init_rollout, n_rollouts + 1):
                ...
                costs_q = costs[-int(8000/args.n_envs):, :].flatten()   # Get the most recent 8000 transitions for the cost critic update
                ...
                for _ in range(args.update_epochs):
                    ...
                    for start in range(0, batch_size, minibatch_size):
                        ...
                        Jc = -(costs_q - args.cost_threshold)
                        loss_cost = (k * F.relu(surr_cost_advantage + Jc)).mean()
                        
                        pg_loss = pg_loss + loss_cost
            ...
            ckpt.set_record(args, agent.actor, agent.critic, agent.cost_critic, global_step, actor_optim, critic_optim, cost_critic_optim, cost_threshold, "" if not logger else logger.wb_path, iteration)
            ckpt.save()
